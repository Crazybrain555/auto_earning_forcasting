...
4. Read `references/profit-forecast-accuracy.md` first - it is the objective function. This method's own measured errors: FY+1/FY+2/FY+3 revenue APE ~6/10/19%, twelve of twelve horizons under-forecast, and in the worst cases >90% of the profit miss came from the MARGIN and below-the-line lines, not from revenue. So: forecast margin as a quantified bridge (never as a percentage), screen the tax/DTA/interest/FX/one-off/share-count layer explicitly, argue any deceleration below trailing growth, set interval widths from the measured error rather than from habit, and build FY+1 as a nowcast of quarters already partly known. Then read `references/driver-tree-modeling.md` (the modeling constitution: historical base -> terminal-demand anchors -> driver tree with volume x price leaves -> explicit main line 主线 -> unit economics -> three statements), then `references/mechanism-router.md` to pick the decomposition template for each branch. Segments must sum to total revenue; the snapshot carries `driver_tree` (mechanism weights are retired - a model is one arithmetic tree, not a weighted blend). Model integrity is mechanical: `references/model-mechanical-integrity.md` (quarterly spine for FY+1, Check rows that tie the three statements, diagnostic qoq/yoy/%rev rails, two independent revenue decompositions). Research breadth is enforced: `references/research-lanes-and-corroboration.md` defines eight lanes (filings, management voice, cross-company, industry data, sell-side, expert/channel, technical literature, trade press), requires >=5 lanes searched with management voice mandatory, and requires every material assumption to be corroborated across >=2 lanes with at least one anchoring lane. Materiality is COMPUTED, never assigned: each assumption states the perturbation tested (test_delta) and what it moves - assigned 0-1 weights are factor scoring and are not used anywhere in this method. Log every search, including the ones that found nothing. Every multi-year forecast contains a persistence claim: read `references/business-quality-and-moat.md` and state the ROIC-WACC spread, an explicit fade assumption argued either way, the company's position in the industry profit pool, and the barrier that blocks the competitive response - with the observation that would show the moat eroding. For any FY+2+ horizon read `references/technology-trend-evidence.md` and build the technology lane (papers/standards/patents -> driver parameter -> falsification condition) - filings are backward-looking and cannot answer which transition lands when.
...
- `references/profit-forecast-accuracy.md`
