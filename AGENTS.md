...
- **Codex installs:** Codex-side skills install at `.agents/skills/` from the same GitHub repo. `.agents/skills/ai-hardware-forecasting` is the pre-split legacy skill; replace it with the two current skills when the Codex port is finalized.
