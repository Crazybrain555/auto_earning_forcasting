...
> 最后更新：2026-07-21 晚（Claude session：MRVL 事故取证 + 本文件建立）。
...
- **runner（aws-sg，档案见 deploy/forecast_runner/README.md）**：skills checkout 是 `ce917c6+dirty`（rsync 上去的未提交内容≈cf46400），需对齐成干净 checkout（待办 #6）；backend 代码版本 9ab2c54，落后本地。
...
- skills：trainer 套件（见 forecasting-skills 内测试；当前基线 556 passed / 1 skipped）
